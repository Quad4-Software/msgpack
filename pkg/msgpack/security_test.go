...
// TestUint32HeaderOverflowGuards exercises the uint32 length decode on
// readers that do not expose Len, where the remaining-input and
// soft-ceiling guards cannot fire. On 32-bit builds a 32-bit length of
// MaxUint32 must be rejected by the int-overflow guard. On 64-bit the
// byte-length APIs return the declared length for the caller to handle.
func TestUint32HeaderOverflowGuards(t *testing.T) {
	t.Run("str32_max", func(t *testing.T) {
		dec := msgpack.NewDecoder(limitedReader{bytes.NewReader([]byte{
			0xdb, 0xff, 0xff, 0xff, 0xff,
		})})
		n, err := dec.DecodeBytesLen()
		assertUint32LenBehavior(t, "str32 length", n, err)
	})

	t.Run("bin32_max", func(t *testing.T) {
		dec := msgpack.NewDecoder(limitedReader{bytes.NewReader([]byte{
			0xc6, 0xff, 0xff, 0xff, 0xff,
		})})
		n, err := dec.DecodeBytesLen()
		assertUint32LenBehavior(t, "bin32 length", n, err)
	})

	t.Run("ext32_max", func(t *testing.T) {
		dec := msgpack.NewDecoder(limitedReader{bytes.NewReader([]byte{
			0xc9, 0xff, 0xff, 0xff, 0xff, 0x01,
		})})
		_, n, err := dec.DecodeExtHeader()
		assertUint32LenBehavior(t, "ext32 length", n, err)
	})

	// Container lengths above the soft ceiling are rejected on every
	// architecture: by the int-overflow guard on 32-bit and by the
	// decode-limit ceiling on 64-bit.
	t.Run("array32_max", func(t *testing.T) {
		dec := msgpack.NewDecoder(limitedReader{bytes.NewReader([]byte{
			0xdd, 0xff, 0xff, 0xff, 0xff,
		})})
		n, err := dec.DecodeArrayLen()
		assertOversizedContainerRejected(t, "array32 length", n, err)
	})

	t.Run("map32_max", func(t *testing.T) {
		dec := msgpack.NewDecoder(limitedReader{bytes.NewReader([]byte{
			0xdf, 0xff, 0xff, 0xff, 0xff,
		})})
		n, err := dec.DecodeMapLen()
		assertOversizedContainerRejected(t, "map32 length", n, err)
	})
}
